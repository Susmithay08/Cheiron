...
> To repeat, these are just some example queries and visualizations possible. You are not restricted to these in your assignment.
...
│   ├── agent/
│   │   ├── planner.py       # LLM-based query interpreter
│   │   ├── tools.py         # ClinicalTrials API tool wrappers
│   │   └── validator.py     # Output validation & constraints
│   ├── api/
│   │   ├── clinicaltrials.py  # CT.gov API client
│   │   └── schemas.py         # Pydantic input/output schemas
│   ├── visualization/
│   │   ├── router.py        # Picks viz type from query intent
│   │   ├── bar_chart.py
│   │   ├── time_series.py
│   │   ├── network_graph.py
│   │   └── histogram.py
│   └── citations/
│       └── tracer.py        # Deep citation builder (bonus)
├── examples/
│   ├── query1_time_trend.json
│   ├── query2_distribution.json
│   ├── query3_comparison.json
│   ├── query4_geographic.json
│   └── query5_network.json
├── tests/
│   └── test_agent.py
├── README.md
...
1. **How does the agent interpret queries?** (e.g., LLM intent classification, regex routing, or structured prompting)
2. **How do you map ClinicalTrials.gov API fields to visualization data?**
3. **How do you handle API pagination / large result sets?**
4. **How do you validate/constrain LLM outputs to avoid hallucination?**
5. **How do you choose the right visualization type for a given query?**

### ClinicalTrials.gov API Quick Reference

Base URL: `https://clinicaltrials.gov/api/v2/`

Key endpoints:
- `GET /studies` — search/filter studies
- `GET /studies/{nctId}` — single study detail
- `GET /stats/field/values` — value distributions for a field
- `GET /stats/size` — total study counts

Useful query params for `/studies`:
- `query.term` — keyword search
- `filter.overallStatus` — e.g., RECRUITING
- `filter.phase` — trial phase
- `postFilter.geo` — geographic filter
- `fields` — comma-separated fields to return
- `pageSize` / `pageToken` — pagination

### Example Queries to Demonstrate

1. **Time trend:** "How has the number of trials for Pembrolizumab changed per year since 2015?"
2. **Distribution:** "How are breast cancer trials distributed across phases?"
3. **Comparison:** "Compare trial counts by phase for Ozempic vs Wegovy"
4. **Geographic:** "Which countries have the most recruiting trials for Alzheimer's disease?"
5. **Network:** "Show a network of sponsors and drugs for diabetes trials"
